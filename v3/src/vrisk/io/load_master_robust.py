...
class RobustMasterDataLoader(MasterDataLoader):
    ...
    def _handle_missing_data_patterns(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        iex_start = datetime(2020, 8, 28, tzinfo=timezone.utc)
        ...
        if iex_cols and 'timestamp' in df.columns:
            ...
            for col in iex_cols:
                df = df.with_columns(
                    pl.when(pl.col('timestamp') < pl.lit(iex_start))
                    .then(None)
        ...
        if sip_cols and 'timestamp' in df.columns:
            ...
            early_cutoff = datetime(2020, 9, 1, tzinfo=timezone.utc)
            early_data = df.filter(pl.col('timestamp') < pl.lit(early_cutoff))
