...
class RobustMasterDataLoader(MasterDataLoader):
    ...
    def _handle_missing_data_patterns(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if iex_cols and 'timestamp' in df.columns:
            ...
            for col in iex_cols:
                df = df.with_columns(
                    pl.when(pl.col('timestamp') < iex_start)
                    .then(None)
        ...
        if sip_cols:
            ...
            early_data = df.filter(pl.col('timestamp') < datetime(2020, 9, 1))
            if len(early_data) > 0:
    ...
    def _report_missing_data(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        if col_stats:
            ...
            logger.info(f"Top missing columns: {[f'{c[0]}: {c[1]['null_pct']:.1f}%' for c in worst_cols]}")
